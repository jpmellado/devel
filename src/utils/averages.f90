...
! Vector form
    subroutine AVG1V2D_V(nx, ny, nz, imom, a, avg, wrk)
        ...
        real(wp), intent(in) :: a(nx, ny, nz)
        real(wp), intent(out) :: avg(nz)
        real(wp), intent(inout) :: wrk(nz)
...
!########################################################################
    function AVG1V2D1G(nx, ny, nz, k, igate, imom, a, gate) result(avg)
        integer(wi), intent(in) :: nx, ny, nz, k, imom      ! Moment order
...
!########################################################################
! Intermittency factor within the plane k
!########################################################################
    function INTER1V2D(nx, ny, nz, k, igate, gate) result(avg)
...
! ###################################################################
! Covariance within the plane k
! ###################################################################
    function COV2V2D(nx, ny, nz, k, a, b) result(avg)
        integer(wi), intent(in) :: nx, ny, nz, k
        real(wp), intent(in) :: a(nx, ny, nz), b(nx, ny, nz)
        real(wp) avg
